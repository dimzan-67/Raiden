...
local ConfigLibrary = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/dimzan-67/Raiden/refs/heads/main/main.lua"))()
...
if identifyexecutor() == "Solara" then -- Quads are broken on Solara.
	local DrawQuad = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/dimzan-67/Raiden/refs/heads/main/main.lua"))() -- Custom Quad Drawing Object
...
if not Degrade then
	...
	warn("RAIDEN_ESP > Your exploit does not support this module's optimizations! The visuals might be laggy and decrease performance.")
end
]=]
...
if RaidenDeveloperESP and RaidenDeveloperESP.Exit then
	RaidenDeveloperESP:Exit()
end

--// Settings

getgenv().RaidenDeveloperESP = {
	DeveloperSettings = {
		Path = "Raiden Developer/Raiden ESP/Configuration.cfg",
...
local Environment = getgenv().RaidenDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("RAIDEN_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "RAIDEN_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("RAIDEN_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "RAIDEN_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "RAIDEN_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "RAIDEN_ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().RaidenDeveloperESP[Index] = nil
		end
		...
		getgenv().RaidenDeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "RAIDEN_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "RAIDEN_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "RAIDEN_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().RaidenDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().RaidenDeveloperESP.Settings = Settings
	getgenv().RaidenDeveloperESP.Properties = Properties

	Environment = getgenv().RaidenDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "RAIDEN_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "RAIDEN_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
